...
print(">>> CONTAINER AVVIATO: Inizio V67.1 (Fix Safetensors + Transfer.sh)...", flush=True)

def install_essentials():
    print(">>> Installazione librerie AI (Incluso Safetensors)...", flush=True)
    # Aggiunto safetensors, kornia e torch che sono fondamentali
    libs = [
        "numpy==1.23.5", "imageio==2.9.0", "imageio-ffmpeg", 
        "opencv-python==4.8.0.74", "edge-tts", "safetensors", 
        "kornia==0.6.8", "tqdm", "yacs", "pyyaml"
    ]
...
def handler(job):
    ...
    # Download modelli AI con bypass SSL
    ...
    try:
        ...
        if result.returncode != 0:
            return {"error": f"Errore inferenza: {result.stderr}"}
